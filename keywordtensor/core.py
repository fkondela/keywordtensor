...
try:
    ...
    class TensorImage:
        def __new__(cls, x, *args, **kwargs): return x
    def show_image(*args, **kwargs): pass

class LoadAudio(Transform):
    def __init__(self, duration=3.0): self.duration = duration
    def encodes(self, file: Path):
        waveform, sr = torchaudio.load(file)
        target_length = int(self.duration * sr)
        if waveform.shape[1] > target_length: waveform = waveform[:, :target_length]
...
class AudioSpectrogram(TensorImage):
    def show(self, ctx=None, **kwargs): return show_image(self, ctx=ctx, cmap='magma', **kwargs)
...
class SpecAugment(Transform):
    ...
    def encodes(self, spec: AudioSpectrogram): return self.fmask(self.tmask(spec))
...
class Engine:
    def __init__(self): self.model_name = None
    def train(self, dataset_path, epochs=30, batch_size=32, wd=0.01, eps=0.01, valid_pct=0.1, model_name='myownmodel', duration=3.0, sr=16000): pass
    def listen(self, model_name, actions=None, min_confidence=0.6, n_averages=3, device=None, source=None):
        ...
        if user_model_path.exists() and user_config_path.exists(): resolved_path = model_name
        elif builtin_model_path.exists() and builtin_config_path.exists(): resolved_path = builtin_base_path
        with open(f"{resolved_path}_config.json", "r", encoding="utf-8") as f: cfg = json.load(f)
        labels = cfg["labels"]
        sr = cfg["sr"]
        ...
        def _run_inference(current_buffer, current_sr):
            ...
            if len(wav_tensor) > buf_len: wav_tensor = wav_tensor[-buf_len:]
            elif len(wav_tensor) < buf_len: return
        ...
        if webrtc_ctx:
            ...
            if not HAS_SOUNDDEVICE: raise RuntimeError("Live listening requires sounddevice.")
            def _audio_callback(indata, frames, time_info, status):
                audio_buffer.extend(indata[:, 0].tolist())
            stream = sd.InputStream(samplerate=sr, channels=1, callback=_audio_callback)
            stream.start()
            current_sr = sr
            def poll_audio(): return current_sr
        try:
            while True:
                if webrtc_ctx and not webrtc_ctx.state.playing: break
